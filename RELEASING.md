...
`maille` ships as a PyPI package (`maille`). Versioning is automated by
...
| --- | --- | --- |
| `fix:` | patch | `fix: keep a small object at every level` |
| `feat:` | minor | `feat: add an LOD planner` |
...
| --- | --- | --- |
| `main` | stable `X.Y.Z` | the default install (`pip install maille`) |
| `next` | prereleases `X.Y.Z-rc.N` | published as a **prerelease** — only reached via `pip install maille --pre` or an exact pin |
| `N.x` (e.g. `2.x`) | maintenance `X.Y.Z` | published stable for an older major |

PyPI marks `…-rc.N` versions as prereleases, so a plain `pip install maille`
...
```sh
pip install maille --pre          # latest rc (or stable, whichever is newer)
pip install 'maille==3.0.0-rc.1'  # pin a specific rc
```

Stable consumers (`pip install maille`) are unaffected by the `next` channel.
